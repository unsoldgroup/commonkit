...
- A Cloudflare API token limited to DNS edit access for the `unsold.cloud` zone, plus the Cloudflare account id.
...
Create this record manually in the Cloudflare `unsold.cloud` zone:

| Type | Name | Value | Proxy |
| --- | --- | --- | --- |
| A | `board` | VPS tailnet IPv4 address | DNS only |

The address must be the VPS Tailscale address, not its public address. Access therefore requires the client to be on the tailnet. Do not expose the hub port or this hostname through a public reverse proxy. The installer does not create or change this record. The Cloudflare credentials passed to acme.sh are used only for the temporary DNS-01 validation record.
...
unset SESSION_BOARD_REPORTER_TOKENS SESSION_BOARD_ACTION_TOKEN CF_Token CF_Account_ID
```
...
2. Add `"target-id":"new-token"` to `SESSION_BOARD_REPORTER_TOKENS` and rerun `install-vps.sh` with the complete token map, action token, and Cloudflare variables. This refreshes the certificate as well as the service configuration.
...
Certificate renewal uses the same DNS-01 flow. Rerun `install-vps.sh` with `CF_Token` and `CF_Account_ID` before expiry, or configure a reviewed local renewal job that runs `acme.sh --renew`, installs the renewed files at `/etc/caddy/certs/session-board/`, and reloads Caddy. Do not place Cloudflare credentials in the hub environment file.
