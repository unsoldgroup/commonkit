...
Audit basis: committed implementation on `uns-1274-commonkit-v1-rust-tauri` through `977e3b5` (`977e3b5f73b554cd7c75222d038f84609860a89b`). Uncommitted lifecycle remediation is deliberately excluded from completion evidence. “Met locally” means the committed Rust production path and its local contract suites pass on the audit host; it does not substitute for an installed lifecycle, production signing/notarization, a published updater, or hosted multi-OS evidence. GitHub reported no workflow run for this exact commit at audit time.
...
| --- | --- | --- | --- | --- |
| 1 | Initialize a target | Met locally | `commonkit init create|connect` creates or clones a kit, writes the composition layers and target registration, establishes private runtime roots, validates pinned native/APM/chezmoi inputs, materializes an initial provider plan, and writes `headless.json`. Desktop onboarding uses the same bounded provider choices. | Installed first-run evidence on the declared platform matrix. |
...
| 3 | Apply safely | Met locally | Plans are content-bound and revalidated before apply. Deterministic adapters provide confirmation, idempotency, receipts, verification, reverse rollback, typed-SSH dispatch, capability-bound filesystem access, and fresh-process reconstruction from durable artifacts. | A passing installed reconciliation lifecycle, followed by signed-artifact execution on all three OSes. |
...
- The current installed-lifecycle attempt did not complete successfully, so the workflow definition and its component tests are not counted as end-to-end installed evidence.
...
1. Make the unsigned installed CLI/daemon lifecycle pass from onboarding through reconcile, verify, snapshot/restore, restart recovery, update handoff, and uninstall. A workflow definition is not execution evidence.
...
1. A green installed-lifecycle run against the current implementation, including daemon restart and snapshot authority initialization.
2. A real local macOS desktop/menu-bar install and isolated VPS acceptance run using the resulting artifacts.
...
Do not mark CommonKit v1 complete or release-ready. The first eight runtime flows and flows 9–11 are locally implemented and covered by committed production-path contracts, but flow 12 remains partial. There is one immediate implementation gate—the installed lifecycle must pass—followed by real Mac/VPS acceptance and the external signing, publication, updater, platform, provider, and remote-target evidence. No P0 finding was identified in this audit.
