...
- `docs/evidence/eight-flow-macos-arm64-a1443c0.json`, the superseded `docs/evidence/eight-flow-macos-arm64-e25785d.json`, and `docs/evidence/eight-flow-linux-x64-e98dee5.json` bind successful macOS arm64 and Ubuntu Linux x86_64 lifecycles to exact commits, platforms, architectures, Node versions, and SHA-256 digests of the four installed binaries. They do not establish a pass for an arbitrary later HEAD. The Linux run used an isolated temporary source tree, Rust toolchain, runtime root, and loopback ports on the Hostinger VPS; the complete temporary directory was removed after retrieving evidence. Future native runs use `scripts/qualify-eight-flows.sh` to emit the same secret-free format.
