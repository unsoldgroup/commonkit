...
| --- | --- | --- | --- | --- |
| Composition, policy, plans, receipts | Implemented | Native manual validation required | Native manual validation required | Transport-neutral |
| Local filesystem reconciliation | Implemented | Native manual validation required | Native ACL/reparse validation required | Typed SSH boundary |
...
| Credentials | env/file/BWS; absolute-path Keychain CLI | env/file/BWS; absolute-path Secret Service CLI | env/file/BWS; native Credential Manager API, native Windows validation required | Target-local only |
...
| Target helper | Packaged and exercised through typed stdin | Packaged; isolated real-`sshd` smoke must be run manually | Packaged for local tooling; remote Windows out of v1 | Required on SSH targets; fixed `commonkit-target-helper --stdio-v1` command |
