...
The standalone CLI is the primary release surface. The desktop GUI is alpha
and does not gate a CLI release. The optional Session Board is outside the core
...
| Desktop GUI (alpha) | Tray health, onboarding, target/loadout management, relay, snapshots, schedules, diagnostics, and updater implemented; signed/notarized alpha lifecycle gated | Same alpha management contract; AppImage/`.deb` installed lifecycle gated | Same alpha management contract; MSI/NSIS installed lifecycle gated | Optional client of the target-local daemon; no desktop is installed on headless targets |
| Standalone CLI (primary) | Universal signed archive gate | Signed archive gate | Signed archive gate | Primary control API |
...
| Cloudflare MCP server portal | Preferred for eligible remote HTTP MCPs; CommonKit adapter designed, not shipped | Same cloud service; adapter designed, not shipped | Same cloud service; adapter designed, not shipped | Same cloud service; portal state is not canonical CommonKit state |
| Local MCP relay fallback | Implemented for stdio, local, private, offline, and portal-incompatible MCPs | Same | Same contracts; native validation required | Target-resident compatibility endpoint |
