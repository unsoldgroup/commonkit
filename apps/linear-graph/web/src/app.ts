...
import type { Campaign, ExecutionRun, WorkBundle } from "@commonkit/linear-graph-protocol";
...
function renderShell() {
  ...
  <main class="layout">
    ...
    <section class="workspace">
      ...
      ${campaignProposal ? `${renderCampaignProposal(campaignProposal)}${campaigns.length > 1 ? `<section class="campaign-summary" aria-live="polite"><div class="campaign-summary-head"><span class="section-kicker">CAMPAIGN HISTORY</span><strong>${campaigns.length} persisted campaigns</strong><span>Reloaded from the hub</span></div>${campaigns.filter((campaign) => campaign.id !== campaignProposal?.id).slice(0, 5).map(renderCampaignHistory).join("")}</section>` : ""}` : campaigns.length ? `<section class="campaign-summary" aria-live="polite"><div class="campaign-summary-head"><span class="section-kicker">CAMPAIGNS</span><strong>${campaigns.length} persisted campaign${campaigns.length === 1 ? "" : "s"}</strong><span>Reloaded from the hub</span></div>${campaigns.slice(0, 5).map(renderCampaignHistory).join("")}</section>` : ""}
  ...
  setTextContent($("#brief-copy"), briefCopy());
  renderRecommendations(); renderZones(); renderList(); renderGraph(); renderContext();
}

function renderCampaignProposal(campaign: Campaign) {
  const bundleCards = campaign.bundles.map((bundle: WorkBundle) => {
    const runs = executions.filter((run) => run.bundleId === bundle.id);
    const evidence = runs.flatMap((run) => run.evidence).slice(-8);
    return `<article class="bundle-card"><div><span class="section-kicker">BUNDLE</span><strong>${esc(bundle.title)}</strong><p>${esc(bundle.summary)}</p><small>${bundle.issueIds.length} tickets · expected reduction ${bundle.expectedReduction}</small>${evidence.length ? `<details class="execution-evidence"><summary>Execution evidence (${evidence.length})</summary>${evidence.map((item) => `<pre>${esc(item.text)}</pre>`).join("")}</details>` : ""}</div><div class="bundle-actions"><span class="campaign-status status-${bundle.status}">${esc(bundle.status)}</span>${bundle.status === "proposed" ? `<button type="button" class="selection-action" data-approve-bundle="${esc(bundle.id)}">Approve</button>` : ""}${bundle.status === "approved" ? `<button type="button" class="selection-action" data-run-bundle="${esc(bundle.id)}">Run agent</button>` : ""}</div></article>`;
  }).join("");
...
function renderCampaignHistory(campaign: Campaign) {
  return `<button type="button" class="issue-row" data-campaign-id="${esc(campaign.id)}"><span class="issue-id">${esc(campaign.id.slice(0, 18))}</span><span class="issue-title">${esc(campaign.title)}</span><span class="issue-topic">${campaign.issueIds.length} issues</span><span class="issue-status">${esc(campaign.status)}</span></button>`;
}
...
async function loadDrainState() {
  try {
    const [drainResponse, executionResponse] = await Promise.all([fetch("/api/work-drain", { cache: "no-store" }), fetch("/api/executions", { cache: "no-store" })]);
    if (drainResponse.ok) {
      const state = await drainResponse.json() as { campaigns?: Campaign[] };
      campaigns = Array.isArray(state.campaigns) ? state.campaigns : [];
      if (!campaignProposal && campaigns[0]) campaignProposal = campaigns[0];
    }
    if (executionResponse.ok) {
      const state = await executionResponse.json() as unknown;
      executions = Array.isArray(state) ? state as ExecutionRun[] : [];
    }
    renderShell();
  } catch {
    // The graph remains usable if the optional persisted drain panel is unavailable.
  }
}
...
async function approveBundle(bundleId: string) {
  ...
  campaigns = campaignProposal ? [campaignProposal, ...campaigns.filter((campaign) => campaign.id !== campaignProposal?.id)] : campaigns;
  toast("Bundle approved; ready for Codex execution"); renderShell();
}
...
async function runBundle(bundleId: string) {
  ...
  const responseBody = await response.json() as { execution?: ExecutionRun; campaignId?: string };
  const execution = responseBody.execution ?? { status: "running" };
  if (responseBody.execution) executions = [responseBody.execution, ...executions.filter((run) => run.id !== responseBody.execution?.id)];
  if (campaignProposal) campaignProposal = { ...campaignProposal, status: "running", bundles: campaignProposal.bundles.map((bundle) => bundle.id === bundleId ? { ...bundle, status: execution.status === "completed" ? "verified" : "running" } : bundle) };
