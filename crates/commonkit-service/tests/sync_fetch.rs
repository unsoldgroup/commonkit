...
impl SyncDomain for ReadOnlyPlan {
...
#[tokio::test]
async fn unconfirmed_read_only_planning_works_for_default_and_target_routes_without_applying() {
    let domain = Arc::new(ReadOnlyPlan {
    ...
    control
        .set_target_sync_domains(std::collections::BTreeMap::from([(
    ...
    for route in [
        ...
        "/control/v1/targets/workstation/sync/plan",
    ] {
        let response = app
            .clone()
            .oneshot(
                Request::post(route)
                    ...
                    .body(Body::from(
                        r#"{"confirmed":false,"confirmationId":"read-only-plan"}"#,
                    ))
                    .unwrap(),
            )
            .await
    ...
    assert_eq!(
        ...
        0,
        "planning must never invoke the apply executor"
    );
}
...
#[tokio::test]
async fn sync_fetch_requires_confirmation_before_git_or_provider_work() {
    let domain = Arc::new(GatedSync {
        git_state: Some("clean"),
    ...
    let response = app
        .oneshot(
            Request::post("/control/v1/sync/plan")
                ...
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(
                    r#"{"confirmed":false,"confirmationId":"sync-fetch","fetch":true}"#,
                ))
                .unwrap(),
        )
        .await
