...
use commonkit_adapters::{
    ...
    ProviderInputs, ProviderPipeline, ProviderPlanRequest, ProviderPlannerRoute,
    ProviderResourcePlanner, ProviderResourceRouter, RemoteProviderStager,
...
impl ProductionSshPlanExecutor {
    ...
    fn adapter(&self) -> Result<Vec<Box<dyn Adapter>>, DomainFailure> {
        ...
        let package_backend: Box<dyn commonkit_adapters::PackageMutationBackend> =
            if let Some(config) = self.target.package_resolution.clone() {
                Box::new(package_backend.with_target_package_resolution(config))
            } else {
                Box::new(package_backend.require_target_package_resolution())
            };
        Ok(vec![
            Box::new(
    ...
    fn execute_inner(
        ...
        match self.receipts.load(run_id.clone()) {
            Ok(_) => self.receipts.with_recovery_lock(|store| {
                let reconciler = Reconciler::with_store_locked(store);
                let journal = reconciler
                    .load_validated_run(run_id.clone(), &durable)
                    .map_err(|_| DomainFailure::OperationFailed)?;
                match journal.receipt().state {
                    ...
                    _ => {
                        ...
                        reconciler
                            .recover_run_with_journal_locked(
                                run_id,
                                &durable,
                                journal,
                                &mut adapters,
                ...
                Some(SyncTargetTransport::Ssh {
                    ...
                    Some((
                        ...
                        ProductionSshTarget {
                            ...
                            package_resolution: sync.target_package_resolution(),
                        },
                    ))
                }
                _ => None,
...
fn validate_sync_config(config: &SyncConfig) -> Result<(), ProductionDomainError> {
    ...
    if let Some(engram) = &config.engram {
        ...
        } else if engram.executable.is_some() {
            return Err(ProductionDomainError::UnsafeConfig);
        }
    }
    Ok(())
...
impl SyncDomain for ProductionSyncDomain {
    fn engram_status(&self) -> Result<Value, DomainFailure> {
        ...
        let target = self.open_engram_target(&self.config, RootAccess::ReadOnly)?;
        let status = EngramChunkAdapter::status_target(target.filesystem(), &declaration)
            .map_err(|_| DomainFailure::VerificationFailed)?;
        serde_json::to_value(status).map_err(|_| DomainFailure::OperationFailed)
    }
    ...
    fn engram_reconcile(&self, request: Value) -> Result<Value, DomainFailure> {
        ...
        let peer = self
            ...
            .ok_or(DomainFailure::InvalidRequest)?;
        let left = Self::engram_declaration(&self.config)?;
        let right = Self::engram_declaration(peer)?;
    ...
    fn rollback(&self, request: Value) -> Result<Value, DomainFailure> {
        ...
        let outcome =
            receipts.with_recovery_lock(|store| -> Result<ReconcileOutcome, DomainFailure> {
                let reconciler = Reconciler::with_store_locked(store);
                let journal = reconciler
                    ...
                    .map_err(|error| match error {
                        ...
                        | commonkit_reconcile::ReconcileError::ReceiptSnapshotChanged => {
                ...
                reconciler
                    .rollback_succeeded_run_with_journal_locked(
                        run_id.clone(),
                        &plan,
                        journal,
                        &mut adapters,
...
mod package_resolution_tests {
    ...
    fn production_resolved_package_stage_emits_a_package_operation_with_authority_binding() {
        ...
        let domain = ProductionSyncDomain {
            ...
            sync_configs: Arc::new(BTreeMap::new()),
        };
