...
use commonkit_adapters::{
    ArtifactStore, ContentSensitivity, DeclaredSideEffect, ExactProviderVersion, FileAdapter, FilesystemIntent,
    MaterializedState, NormalizedManagedPath, NormalizedResource, OwnershipRules, ProviderInputs,
    ProviderPlanError, ProviderPlanRequest, ResourceProvenance, UnsupportedCapability,
    build_provider_plan,
};
...
fn verified_provider_states_build_a_durable_mutation_free_bound_plan() {
    ...
    let apm_content = artifacts
        .put(b"apm\n", ContentSensitivity::Portable)
        .unwrap();
    let native = state("native", "1.0.0", "native-v1", "home/native.txt", native_content);
    ...
    let plan = build_provider_plan(request, &[apm.clone(), native.clone()], &artifacts, &mut adapter)
    ...
    assert_eq!(fs::read(target.join("home/native.txt")).unwrap(), b"native\n");
    ...
    assert_ne!(changed_plan.bindings.provider_inputs_digest, plan.bindings.provider_inputs_digest);
...
fn provider_policy_failures_stop_before_operation_registration() {
    ...
    let target = root.join("target");
    let artifacts = ArtifactStore::open(root.join("provider-artifacts")).unwrap();
    let content = artifacts.put(b"data", ContentSensitivity::Portable).unwrap();
    let base = state("native", "1.0.0", "native-v1", "outside/file", content);
    let rules = OwnershipRules::new(true, vec![NormalizedManagedPath::parse("home").unwrap()], vec![]).unwrap();
    let request = || ProviderPlanRequest {
        target_id: StableId::parse("local").unwrap(),
        composed_loadout_digest: digest('a'), observed_digest: digest('b'), policy_digest: digest('c'),
        ownership_rules: &rules, mapped_side_effects: BTreeSet::new(),
    };
    let error = build_provider_plan(request(), &[base.clone()], &artifacts,
        &mut FileAdapter::open(&target, &root.join("outside-state")).unwrap()).unwrap_err();
    assert!(matches!(error, ProviderPlanError::Ownership(_)));

    let unsupported = MaterializedState::finalize(base.inputs.clone(), vec![], vec![], vec![UnsupportedCapability {
        source: "run_once.sh".into(), capability: "script".into(), remediation: "remove it".into(),
    }]).unwrap();
    let error = build_provider_plan(request(), &[unsupported], &artifacts,
        &mut FileAdapter::open(&target, &root.join("unsupported-state")).unwrap()).unwrap_err();
    assert!(matches!(error, ProviderPlanError::UnsupportedCapability { .. }));

    let side_effect = MaterializedState::finalize(base.inputs, vec![],
        vec![DeclaredSideEffect::ServiceRestart { service: "editor".into() }], vec![]).unwrap();
    let error = build_provider_plan(request(), &[side_effect], &artifacts,
        &mut FileAdapter::open(&target, &root.join("effect-state")).unwrap()).unwrap_err();
